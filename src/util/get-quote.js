...
const quotes= {
  quoteArray: [
    ...
    [
      ...
      `Tsk, tsk. That's hardly complimentary, Highness. Why loose your venom on me?`,
      `You killed my love.`,
      `It's possible. I kill a lot of people.`,
    ],
    [
      `It was ten days to the wedding. The King still lived, but Buttercup's nightmares were growing steadily worse.`,
    ...
    [
      ...
      `Won't that be nice? She kissed me! Ha!`,
    ],
    [
    ...
    [
      `Where am I?`,
      `The Pit of Despair! Don't even think… ... don't even think about trying to escape. The chains are far too thick. Don't dream of being rescued, either; the only way in is secret. Only the Prince, the Count, and I know how to get in and out.`,
    ],
    [
...
/**
 * Selects random quote from an array of quotes.
 * @param {*} quoteArray
 * @returns Object at random index of quote array. May return another array of quotes or a single quote.
 */
const randSelector = quoteArray => {
  return Math.floor(Math.random() * quoteArray.length);
};

const GetQuote = () => {
  return quotes.quoteArray[randSelector(quotes.quoteArray)];
}

// Returns a single sentence quote.
const GetSingleLine = () => {
  const quote = GetQuote();
  let newQuote;
  // Check if quote is an array. If so, take random object from array.
  if (Array.isArray(quote)) {
    newQuote = quote[randSelector(quote)];
  }
  const punctuation = [ '.', '?', '!', '...'];
  // Assume the quote is a single sentence.
  let stoppingPoint = newQuote.length;
  // Iterate over the quote to find the first punctuation mark.
  for (let p of punctuation) {
    let found = newQuote.indexOf(p);
    if (found < stoppingPoint && found > -1) {
      stoppingPoint = found + 1;
    }
  }
  // Return the first sentence of the quote
  return newQuote.slice(0, stoppingPoint);
}

const Quotes = { GetQuote, GetSingleLine };
